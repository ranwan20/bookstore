...
def search_book():
    ...
    b = Buyer()
    code,message,result = b.search_book(key,page)
    return jsonify({"message": message,"result":result}), code

@bp_buyer.route("/search_book_in_store", methods=["POST"])
def search_book_in_store():
    store_id: int = request.json.get("store_id")
    key: str = request.json.get("key") 
    b = Buyer()
    code,message,result = b.search_book_in_store(store_id,key)
    return jsonify({"message": message,"result":result}), code
